...
#### Remove Variables with Zero Variance, and high NA's
...
```{r}
# remove variables that are Unique Identifiers  (columns 1 to 5)
...
We use the correlation plot to observe the relation among the variables. 
...
We shall use 3 different methods, and compare the accuracy of each one of them, before deciding the final model.
Confusion matrix of each model is also plotted, to give a holistic picture of the model .
...
It is clearly evident that the RF seems to be better classifier, so we use that for the test sets given for the exercise. 
